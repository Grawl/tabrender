...
// Phone layout for the bottom toolbar (`div.toolbar > div.scroll`, upstream flex row with horizontal scroll). Below 640px width the row wraps, via flexbox `order` rather than DOM reordering: Play and the three switch buttons (Loop, Count in, Metronome) first, then the track/audio selectors, then the speed menu button, then Edit (only present when logged in), each group dropping to its own line once it no longer fits — the toolbar's own height goes from a fixed 44px to auto so it can grow with the wrapped content. The switch buttons additionally get an iOS-style toggle knob drawn with a single `::before` pseudo-element instead of upstream's plain `.active` background, since the compact wrapped row leaves little room for a text-only active state; their padding/gap/font-size are shrunk from desktop so Play plus all three switches fit on one line at a 390px viewport. The knob uses a `>` (direct-child) selector so it only targets the switch buttons that are immediate children of `.scroll`, not the Edit button (which upstream nests inside `div.btn-edit`). The toolbar is `position: fixed` at the viewport bottom (upstream, unchanged here) and now taller than the desktop 44px, so `.main` (the score's containing block) gets bottom padding to match, or the fixed toolbar would permanently cover the tail of a scrolled-to-the-end score. The navbar's own right padding is shrunk to match on phones (`.mobile .my-navbar .toolbar[data-v-8f474ce9]{padding:0 0 0 10px}` upstream drops it entirely).
//
