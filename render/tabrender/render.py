...
from . import midi_events, fluid
from .guitar import GuitarKit, AmpChain
from .drums import KICK_NOTES, synth_kick, limiter
...
def render(gp_path: str, out_mp3: str, config: dict | None = None) -> dict:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for t in meta["tracks"]:
            ...
            if chain_key not in chains:
                chains[chain_key] = AmpChain(
                    ...
                    os.path.join(ASSETS, cfg["ir"]), SR,
                    input_gain=cfg["dist_input_gain" if dist else "clean_input_gain"],
                    pre_hpf=cfg["dist_pre_hpf"] if dist else 0, post_hpf=cfg["dist_post_hpf"] if dist else 0,
        ...
        for g, chans in groups.items():
            ...
            if g == "drums":
                ...
                if os.path.exists(kit_path):
                    ...
                    add_stem(fluid.render(midi_path, chans, SOUNDFONT, SR, exclude_notes=KICK_NOTES), cfg["level_drums"])
        ...
        sf.write(wav, mix, SR)
        subprocess.run(["ffmpeg", "-y", "-loglevel", "error", "-i", wav, "-codec:a", "libmp3lame", "-b:a", "192k", "-f", "mp3", out_mp3], check=True)
    info = {"seconds": round(length, 1), "render_time": round(time.time() - t0, 1), "tracks": [t["name"] for t in meta["tracks"]]}
    log("done", info)
    return info
