...
def limiter(x: np.ndarray, sr: int, threshold: float = 0.89, attack: float = 0.001, release: float = 0.08) -> np.ndarray:
    ...
    a = np.exp(-1.0 / (attack * sr)); r = np.exp(-1.0 / (release * sr))
    g = np.empty_like(env); e = 0.0
